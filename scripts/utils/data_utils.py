...
def convert_pose_seq_to_dir_vec(dataset_name, pose):
    if dataset_name == "aqgt":
        ...
        if len(pose.shape) == 3:
            dir_vec = np.zeros((pose.shape[0], len(dir_vec_pairs_expressive), 3))
            for i, pair in enumerate(dir_vec_pairs_expressive):
                dir_vec[:, i] = pose[:, pair[1]] - pose[:, pair[0]]
                dir_vec[:, i, :] = normalize(dir_vec[:, i, :], axis=1)  # to unit length
        elif len(pose.shape) == 4:  # (batch, seq, ...)
            dir_vec = np.zeros((pose.shape[0], pose.shape[1], len(dir_vec_pairs_expressive), 3))
            for i, pair in enumerate(dir_vec_pairs_expressive):
                dir_vec[:, :, i] = pose[:, :, pair[1]] - pose[:, :, pair[0]]
            for j in range(dir_vec.shape[0]):  # batch
                for i in range(len(dir_vec_pairs_expressive)):
...
def convert_dir_vec_to_pose(dataset_name, vec):
    if dataset_name == "aqgt":
        org_shape = vec.shape
